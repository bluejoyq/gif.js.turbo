...
import { NeuQuant } from './NeuQuant.ts';
import { LZWEncoder } from './LZWEncoder.ts';
function ByteArray() {
  this.page = -1;
  this.pages = [];
  this.newPage();
...
function getL2RGBDistance(frame1, frame2, startIdx) {
  ...
  return (
    squareVal(frame1[startIdx] - frame2[startIdx]) +
...
function getRGBDistance(frame1, frame2, startIdx) {
  return (
    Math.abs(frame1[startIdx] - frame2[startIdx]) +
...
GIFEncoder.prototype.addFrame = function (imageData, previousImageData) {
  ...
  if (previousImageData) {
    previousFramePixels = getImagePixelsFromFrame(
      previousImageData,
      this.width,
      this.height,
    );
...
GIFEncoder.prototype.indexPixels = function (previousFrame) {
  ...
  let k = 0;
  let pixelsSameInTheFrame = 0;
  for (let j = 0; j < nPix; j++) {
    let index = -1;
    // Only execute if transparent option available.
    if (
      previousFrame &&
      getRGBDistance(this.pixels, previousFrame, k) <
...
GIFEncoder.prototype.cropIndexedPixels = function () {
  ...
  while (this.yOffset < this.yEnd) {
    ...
    for (let i = 0; i < this.width; i++) {
      if (
        this.indexedPixels[this.width * this.yOffset + i] !==
        this.transIndexValue
      ) {
        isTransparent = false;
        break;
      }
  ...
  while (this.yEnd > this.yOffset) {
    ...
    for (let i = 0; i < this.width; i++) {
      if (
        this.indexedPixels[this.width * this.yEnd + i] !== this.transIndexValue
      ) {
        isTransparent = false;
        break;
      }
  ...
  while (this.xOffset < this.xEnd) {
    let isTransparent = true;
    for (let i = this.yOffset; i < this.yEnd; i++) {
      if (
        this.indexedPixels[this.width * i + this.xOffset] !==
        this.transIndexValue
      ) {
        isTransparent = false;
        break;
  ...
  while (this.xEnd > this.xOffset) {
    ...
    for (let i = this.yOffset; i < this.yEnd; i++) {
      if (
        this.indexedPixels[this.width * i + this.xEnd] !== this.transIndexValue
      ) {
        isTransparent = false;
        break;
      }
...
GIFEncoder.prototype.calculateAndPropogateErrorDither = function (
  frame,
  x,
  y,
  kernel,
  ...
  const direction = serpentine ? -1 : 1;
  ...
  const idx = this.findClosestRGB(r1, g1, b1);
  // Multiply by 3 because color tab will have r.g.b.
  k = idx * 3;
  const r2 = this.colorTab[k];
  const g2 = this.colorTab[k + 1];
  const b2 = this.colorTab[k + 2];
  ...
  for (
    let i = direction == 1 ? 0 : ds.length - 1,
      end = direction == 1 ? ds.length : 0;
    i !== end;
    i += direction
  ) {
    const x1 = ds[i][1]; // *direction;  //  Should this by timesd by direction?..to make the kernel go in the opposite direction....got no idea....
    const y1 = ds[i][2];
    if (
      x1 + x >= 0 &&
      x1 + x < this.width &&
...
GIFEncoder.prototype.ditherPixels = function (
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (
      ...
      index = y * width + x;
      let idx = this.calculateAndPropogateErrorDither(
        this.pixels,
        x,
        y,
        kernel,
      ...
      if (previousFrame) {
        const prevFrameIdx = this.calculateAndPropogateErrorDither(
          previousFrame,
          x,
          y,
          kernel,
        ...
        if (
          idx === prevFrameIdx ||
          getL2RGBDistance(this.pixels, previousFrame, index * 3) <
            this.transparencyDifferenceThreshold *
              this.transparencyDifferenceThreshold
        ) {
...
GIFEncoder.prototype.writeImageDesc = function () {
  const height = this.yEnd - this.yOffset + 1;
  const width = this.xEnd - this.xOffset + 1;
...
GIFEncoder.prototype.writePixels = function () {
  ...
  const enc = new LZWEncoder(width, height, indexedPixels, this.colorDepth);
  enc.encode(this.out);
};
